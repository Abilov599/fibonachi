...
class FibonacchiSearch:
    ...
    def get_fibonacchi_number(self, k):
        first_number = 0
        second_number = 1
        n = 0
        while n < k:
    ...
    def start_init(self, sequince):
        ...
        while (self.get_fibonacci_number(k+1) < len(sequince)):
            k = k + 1
            m = self.get_fibonacchi_number(k+1)-(n-1)
            self.i = self.get_fibonacchi_number(k) - m
            self.p = self.get_fibonacchi_number(k-1)
            self.q = self.get_fibonacchi_number(k-2)
